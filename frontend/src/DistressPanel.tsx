...
function SignalCard({ signal, refresh }: { signal: Signal; refresh: () => Promise<void> }) {
  ...
  return <article className="signal-card"><h3>{signal.kind} · {signal.priority.replaceAll('_', ' ')}</h3><p>{signal.status.replaceAll('_', ' ')} · DEMO</p>
    ...
    {signal.kind === 'VOICE' && <p>Voice evidence requires authority review. Create a reviewed incident report before approving a response.</p>}
    ...
    {signal.report_id && <><a href={'/command?report=' + signal.report_id}>Open incident and simulated assignment</a><details><summary>Authority escalation</summary><p>Assign an available simulated unit. Existing assignments remain recorded; no emergency service is contacted.</p>{['POLICE', 'AMBULANCE', 'FIRE_ENGINE', 'RESCUE_TEAM'].map(type => <button key={type} disabled={busy} onClick={() => void update(type)}>{type.replaceAll('_', ' ')}</button>)}</details></>}
...
export default function DistressPanel() {
  ...
  return <section className="distress-panel" aria-label="Priority distress signals"><h2>Priority distress signals · simulation</h2>{error && <p role="status">{error}</p>}{data.map(signal => <SignalCard key={signal.id} signal={signal} refresh={refresh} />)}</section>;
}
