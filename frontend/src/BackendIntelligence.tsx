...
export default function BackendIntelligence({ result, condensed = false }: { result: AegisResult; condensed?: boolean }) {
  ...
  return <section className={"backend-intelligence" + (condensed ? " condensed" : "")} aria-label="Backend incident intelligence">
    ...
    <details><summary>Why this severity and confidence?</summary>
      <div className="factor-grid">
        <div><strong>Severity contributions</strong>{Object.entries(result.severity_analysis.factors || {}).filter(([, value]) => value > 0).map(([key, value]) => <p key={key}>{key.replaceAll("_", " ")} <b>+{value}</b></p>)}</div>
        <div><strong>Evidence contributions</strong>{Object.entries(result.confidence_analysis?.factors || {}).map(([key, value]) => <p key={key}>{key.replaceAll("_", " ")} <b>+{value}</b></p>)}</div>
      </div>
    </details>
    {!condensed && <>
      ...
      <div className="staging-recommendations"><h3>PRE-POSITION SPARE RESOURCES</h3>
