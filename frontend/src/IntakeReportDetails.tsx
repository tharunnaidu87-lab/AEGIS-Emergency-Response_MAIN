...
export default function IntakeReportDetails({ report }: { report: SharedReport }) {
  ...
  return <details className="command-intake-record" key={report.id}>
    ...
    {report.intake ? <>
      ...
      <p>NLP provider: {report.intake.nlp_provider || 'LOCAL'} | {report.intake.nlp_model || 'Local rules'} | {report.intake.nlp_method || report.intake.extraction.method}</p>
      ...
      {Object.entries(report.intake.citizen_corrections || {}).map(([key, correction]) => <p key={key}>Citizen correction - {key.replaceAll('_', ' ')}: {display(correction.original)} → {display(correction.reviewed)}</p>)}
